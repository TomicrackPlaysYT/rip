const { AccessAccounts } = require("./Scen2Accounts.js");
...
const access = {
  testing: [
    ...
    lv9: [ //Lead developer (kingblade/absentpopcorn33) //Cant this just be all the commands instead of a list? //sure
    "epilepsy",
    "shutdown",
    "restart",
    "s",
...
const BotPermList = {
  Booster: access.lv1,
  Contributor: access.lv2, 
  Youtuber: access.lvl3,
  Featured: access.lvl4,
  Mod: access.lv5,
  Admin: access.lv6,
  Manager: access.lv7,
  Dev: access.lv8,
...
const main = function (tankData, args) {
    ...
    app = function ($, e) {
      ...
      switch(a){
        ...
        case "/account":
          e.setHeader("Access-Control-Allow-Origin", "*");
          if(t.query) {
              AccessAccounts.GetData(Object.fromEntries(
                t.query.split("&").map((value) => value.split("="))
              )["u"]).then((AccountData) => {
                console.log(AccountData)
                if(!AccountData) return e.writeHead(404), e.end("404 Not Found");
                e.setHeader("Content-Type", "application/json");
                e.end(JSON.stringify(AccountData));
              })
          } else e.writeHead(404), e.end("404 Not Found");
          break;
        case "/leaderboard":
          return e.writeHead(404), e.end("404 NOT FOUND");
          break;
        default:
          try {
      let fileToGet = path.join("./public", $.url);

      //if this file does not exist, return the default;
      if (!fs.existsSync(fileToGet)) {
        fileToGet = path.join("./", "index.html");
      } else if (!fs.lstatSync(fileToGet).isFile()) {
        fileToGet = path.join("./", "index.html");
      }

      //return the file
      if (!fs.existsSync(fileToGet)) return (e.writeHead(404), e.end('<script>document.write(`<a style="font-family:monospace" href="${\'https://rip2scenexe.glitch.me?s=\' + new URL(location.href).host}">connect to private server</div>`)</script>'));
      e.setHeader('Content-Type', mimeSet[fileToGet.split('.').pop()] || 'text/html');
      let acceptEncoding = $.headers['accept-encoding'];
      if (!acceptEncoding) {
        acceptEncoding = '';
      };
      if (acceptEncoding.match(/\bgzip\b/)) {
        e.writeHead(200, { 'content-encoding': 'gzip' });
        fs.createReadStream(fileToGet).pipe(zlib.createGzip()).pipe(e);
      } else if (acceptEncoding.match(/\bdeflate\b/)) {
        e.writeHead(200, { 'content-encoding': 'deflate' });
        fs.createReadStream(fileToGet).pipe(zlib.createDeflate()).pipe(e);
      } else {
        e.writeHead(200, {});
        fs.createReadStream(fileToGet).pipe(e);
      }
      return
    } catch (e) { console.error(e) }

          /*(e.writeHead(800, { "Content-Type": "text/html; charset=utf-8" }),
          e.end(
            '<script>document.write(`<body><a style="font-family:monospace" href="${\'https://scenexe2.io?s=\' + new URL(location.href).host}">start playing!<br><a style="font-family:monospace" href="https://discord.gg/ZGMVMFTHHf">Discord</body>`)</script>'
      ));*/
          break;
      }
    },
  ...
  let JSON_tankData = JSON.stringify(tankData),
    Detector = (function () {
      let $ = {
        ...
        detectCollisions: function (e, t, a) {
          ...
          for (let o = 0; o < s; o++) {
            let r = e[o],
              d = r.size || r.w, //values switched
              c = r.size || r.h; //values switched
            (r.left = r.x - d),
              (r.right = r.x + d),
              (r.bottom = r.y - c),
              (r.top = r.y + c),
    ...
    dimension = {
      clipSize: 100, //DEFAULT IS 100
      tickTime: 0.02,
      tickRate: 50,
      tickMultiplier: 2,
      power97: 0.9409, //DEFAULT IS 0.9409
      savedWalls: {
        m1: [
          [-1599, 1501, 1400, 1500],
          [1599, 1499, 1400, 1500],
      ...
      getBulletSpeed: function ($, e) {
        let t = 4.5 * e.sBulletSpeed * $.speed * (1 + e.upgrades[1] / 30);
        return (
          1 === $.type || 3 === $.type
            ? (t *= 1.5)
            : (2 === $.type || 4 === $.type) &&
      ...
      wallRestitution: 0.1, //default is 0.1
      ...
      confine: function ($, e, t) { //im tryna make it more like arras rn (their confineing thing is normal, but gets stronger the farther u go out of bounds)
        e += 0
        $.x < -e
          ? (($.x = -e),
            ($.xv = Math.abs($.xv * dimension.wallRestitution) + t)) //i think when u change value it breaks in some way
          : $.x > e &&
            (($.x = e),
            ($.xv = -Math.abs($.xv * dimension.wallRestitution) - t));
        $.y < -e
          ? (($.y = -e),
            ($.yv = Math.abs($.yv * dimension.wallRestitution) + t)) //is T the variable thats messing up wall collision / probably not I thinks it the wall change speed
          : $.y > e &&
            (($.y = e),
            ($.yv = -Math.abs($.yv * dimension.wallRestitution) - t)); //yeah t is probably the thing where is it defined / just find confine and you should find it
      },
      //rad multi management //if the tanks rad is less than -1 then the score value goes up by the power of 12500 //are you trying to inflate everything //no the chance of that even happening is in the billions // 2 score would be infinity //oh... //i meant multiplied by 12500 not power thats a mb //ok
      getRadiantMultiplier: function ($) {
        return $ <= 0 && $ >= 0
          ? 1
          : $ <= 1 && $
          ? $ < 0 //how much per - rad level does it go up by //afk
            ? $ == -1
              ? 1500
              : $ < -1
              ? 1500 * Math.pow(9, Math.abs($))
              : 1500 * Math.abs($) // Math.args(12500, $) //tf is Math.args??? // i put wrong one in on accident bc i forgor
            : 1 === $
            ? 25
            : Math.pow(25, $)
          : 25 * Math.pow(4, $ - 1);
      ...
      create: function ($) {
        if (!($.name in dimension.dims)) {
          ...
          let i = {
            ...
            maxPolygonCount: t,
            _maxPolygonCount: t,
            maxRadiancy: $.maxRadiancy || 0,
            _maxRadiancy: $.maxRadiancy || 0,
            gleaming: $.gleaming || 0,
            name: $.name,
            tanks: [],
            type: $.type || "ffa",
      ...
      collideWall: function ($) {
        let e = $.rect,
          t = $.circle.object,
            timeout;
        if("bullet" === $.circle.type)(timeout = setTimeout(
          function(){t.remove ? t.remove() || clearTimeout(timeout):clearTimeout(timeout)}, 100));
        else
        if ($.cinX || $.cinY) {
          if ($.cinX && $.cinY)
        ...
        if (
          ...
          1 === t.gateType
            ...
            ? e.level >= 60 && 7 !== e.team
              ...
              : e.remove()
            : 4 === t.gateType
            ? e.score >= 18304787571 && 7 !== e.team
              ? 8 !== e.team &&
                (6 === 6 ? e.abyssalize() : e.abyssalize(),
                e.ws &&
                  e.ws.sendPacket &&
                  e.ws.sendPacket("setStats", e.upgrades))
              : "tank" === e.objectType
      ...
      // radiant name generator START
      getRadiantName: function ($) {
        return $ < 1 && $ > -1
          ? ""
          : $ <= -1
          ? $ > -5
            ? ["Epilepsy", "Flashing", "Decieving", "Blinding"][
                Math.abs($) - 1
              ]
            : `Exostrous (${$})` //i used an for the auto adder, and it might look bad on some of the names
          : $ < 5
          ? ["Radiant", "Gleaming", "Luminous", "Lustrous"][$ - 1]
          : `Highly Radiant (${$})`; //idk how to add this because then it invalidates //the highly rad thing? //yeah //if i try to add a : it gives error
      },
      // radiant name generator END
      //polygon naming
        ...
        return (
          e
            ...
            ? //EPILEPSY NAMER
              $.radiant <= -1 &&
              $.radiant > -2 &&
              (a = `an ${dimension.getRadiantName($.radiant)} `)
            : (a =
                $.radiant < -1
                  ? 0 < "AEIOU".indexOf(t[0])
                    ? "a"
                    : "an"
                  : `a ${dimension.getRadiantName($.radiant)} `)
            ? // FLASHY AND ABOVE NAMER
              $.radiant <= -2 &&
              (a = `a${dimension.getRadiantName($.radiant)} `)
            : (a =
                $.radiant < -2
                  ? 0 < "AEIOU".indexOf(t[0])
                    ? "a"
      ...
      update: function ($, e, t) {
            ...
            d[1].sendPacket &&
              ...
              d[1].data.isPlayer &&
                ...
                (d[0].dim.broadcast(`${d[1].data.tank.name} has spawned.`),
                  /*d[1].sendPacket(
                    "announcement",
                    `Then login using the button on the homepage.`
                  ),
                 !d[1].accountName &&
                  d[1].sendPacket(
                    "announcement",
                    `to create an account: /createaccount {username} {password}`
                  ),*/
                d[1].accountName &&
                  d[1].accountData &&
        ...
        if (
          ("2teams" === $.type || ("4teams" === $.type && $.isMain) || $.isMain) &&
          1 === D.length &&
          D[0] &&
          j.length >= 13
        ) {
        ...
        for (let $B = $.gates.length - 1; $B >= 0; $B--) {
          ...
          0 === $M[3] || 2 === $M[3]
            ? (($C = 30), ($I = $M[4]))
            : (($C = $M[4]), ($I = 30));
          $ASS ? (($M[6] = 120), ($ASS = false)) : false;
          $M[0] === 4 ? ($ASS = true) /*, console.log($M[0])*/ : false;
          let $8 = {
            gateType: $M[0],
            x: $M[1],
            y: $M[2],
          ...
          //          (/*$M[6] !== 120 && */$M[6]) ? console.log($M[6], $M[0], $M[1], $M[2]): false // gate degubbing
          x.push($8);
        }
        ...
        for (let el = $.bullets.length - 1; el >= 0; el--) {
          ...
          if (3 === e_.type || 4 === e_.type || 5 === e_.type) {
        ...
        for (let eG = $.tanks.length - 1; eG >= 0; eG--) {
          ...
          if (eK.health <= 0) {
            ...
            if (
              ...
              if (
                (eK.ws && eK.ws.data.isPlayer && eK.name 
                 && 
                 createMessage(
                    "1264078609673355345",
                    "`" +
    ...
    generator = {
        ...
        let e = function (t) {
          ...
          let a =
            t.data.relativeSizeS >= 0
              ...
              : t.data.width;
          if (t.data && (3 === t.data.type || 4 === t.data.type || 5 === t.data.type)) {
      ...
      getLevel: function ($) {
        return (
          Math.floor(
            Math.round(1e6 * Math.log($ / 500 + 1) * generator.log1) / 1e6 //original value is 500
          ) + 1
        );
      },
      ...
      getAbyssalLevel: function ($) {
        //level 101 - 41408986761
        return (
          Math.floor(
            Math.round(
              1e6 *
                Math.log(($ - 34507488885) / 5e9 + 1) *
                generator.log1
            ) / 1e6
          ) + 120
        );
      ...
      tank: function ($, e) {
        ...
        let a = {
          ...
          abyssalize: function () {
            ...
            (a.upgrades = [15, 15, 15, 15, 15, 15, 15, 15]),
              a.countUpgrades(),
              (a.weapon = "developer"),
              (a.body = "developer"),
              (a.score = 0),
          ...
          update: function () {
            ...
            if (a.celestial)
              ...
              a.score < 34507488884 && (a.score = 34507488884),
                (a.level = generator.getAbyssalLevel(a.score));
            else {
            ...
            if (
              ...
              (a.levelMultiplier = Math.pow(
                1.01,
                a.level - (a.celestial ? 15 : a.abyssal ? 20 : 1)
              )),
              //SIZE SCALE 3 END
              //default size is 30 i think
              (a.size =
                30 *
                Math.pow(
                  a.dim.sizeScale,
                  a.level - (a.celestial ? 0 : a.abyssal ? 0 : 1)
                ) *
                (a.celestial ? 1.5 : 1) *
                //                (a.abyssal ? 1.5 : 1) * //causing problems :(
                e),
              a.size > 1e5 && (a.size = 1e5),
              a.barrels)
            )
      ...
      bullet: function ($) {
        ...
        if (3 === e.data.type || 4 === e.data.type || 5 === e.data.type) {
          ...
          if (r) {
            ...
            for (let d = 0, c = r.turrets.length; d < c; d++) {
              ...
              let m = u.sectionWeapon
                ...
                : u.barrels || [];
              (u.barrels = []), (u.relativeSize = 1), (u.size = 1);
              for (let p = 0, f = m.length; p < f; p++) {
                let g = m[p];
      ...
      polygon: function ($, e) {
        if (
          (!$.dim.sandbox && $.dim.name !== "void" &&
            (($.sides < 0 || $.radiant > 4 || $.score >= 1e9 || $.radiant < 0) &&
                createMessage(
                  "1264078593127092316",
                  `\`[${
      ...
      gameStart: function ($) {
        ...
        for (let a = e.tanks.length - 1; a >= 0; a--) {
          ...
          t.push([
            ...
            n.weapon,
            n.body,
            n?.ws?.accountName ?? n?.ws?.accountData?.username ?? false,
          ]);
        }
        ...
        return [
          ...
          !!e.fillWalls,
          k,
          //this might be important later.. idk
          ...
          for (let y in $.dim.updatedTanks) {
            let h = $.dim.updatedTanks[y],
              k = [
                ...
                h.weapon,
                h.body,
                h?.ws?.accountName ?? h?.ws?.accountData?.username ?? false,
              ];
            g.push(k);
          }
          e.push(g);
    ...
    commands = {
      ...
      rules: {
        ...
        portal: [
          [
            ...
            function ($, e, t) {
              ...
              let command5 = $[6];
              //wormhole gen
              if(!(["/portal", "/removewormhole", "/uploadportals"].includes(command))){
              let wormhole = generator.wormhole({
                x: pos[0],
                y: pos[1], //any portal related commands shouldnt be able to be used in the portal command (/portal /removeworm /uploadportals)
                size: size || 40,
                type: 2,
                dim: e.dim,
                action: function (tank) {
                  if (command, command2, command3, command4, command5) commands.execute(command, command2, command3, command4, command5, tank, tank.ws, true);
                },
              });
              if (command, command2, command3, command4, command5) wormhole.ExecCommand = command;
              }
            },
          ],
        ],
          ...
          [
            ...
            function ($, e, t) {
              ...
              if (Input === "wormholes") {
                ...
                let Id = parseInt(Input.slice(1));
                console.log("Wormhole Target Started");
                for (let B in W) {
                  let M = W[B];
                  if (M.id === Id) {
                    Targets.push(M);
        ...
        downloadportals: [
          [
            [],
            function ($, e, t) {
              let wormholes = e.dim.wormholes;
              let WormholeList = new Array();
              for (let index in wormholes) {
                let wormhole = wormholes[index];
                if (wormhole.type === 2) {
                  WormholeList.push([
                    wormhole.x,
                    wormhole.y,
                    wormhole.size,
                    wormhole.ExecCommand,
                  ]);
                }
              }
              WormholeList.length > 0 &&
                (t.sendPacket(
                  "eval",
                  `navigator.clipboard.writeText(${JSON.stringify(
                    JSON.stringify(WormholeList)
                  )})`
                ),
          ...
          [
            ...
            function ($, e, t) {
              ...
              WormholeList.length > 0 && $[0].sendPacket &&
                ($[0].sendPacket(
                  "eval",
                  `navigator.clipboard.writeText(${JSON.stringify(
                    JSON.stringify(WormholeList)
                ...
                $[0].sendPacket("announcement", "Copied to clipboard"));
            },
          ],
        ],
        uploadportals: [
          [
            ...
            function ($, e, t) {
              try {
                for (let i in e.dim.wormholes) {
                  e.dim.wormholes[i]?.remove?.();
                }
                let portals = JSON.parse($[0]);
                if (portals && portals.length > 0) {
                  for (let i in portals) {
                    let portal = portals[i];
                    let [pX, pY, Si, Co] = portal;
                    //wormhole gen
                    if(!["/portal", "/removewormhole", "/uploadportals"].includes(Co)){
                    let wormhole = generator.wormhole({
                      x: pX,
                      y: pY,
                      size: Si || 40,
                      ...
                      dim: e.dim,
                      action: function (tank) {
                        if (Co) commands.execute(Co, tank, tank.ws, true);
                      },
                    });
                    if (Co) wormhole.ExecCommand = Co;
                    }
                  }

                  t.sendPacket("announcement", `Success: Loaded ${portals.length} portals`);
        ...
        access: [
          [
            ...
            function ($, e, t, perms) {
              let targets = $[0];
              let commands = $[1].replace(/[ ]+/g).split(",");
              t.sendPacket(
                "announcement",
                "You have granted: " + targets.length + " targets: " + commands
              );
              let commandsFail = [];
              for (let target of targets) {
                for (let command of commands) {
                  if (perms || t.data.commands[command])
                    target.ws.data.commands[command] = true;
                  else commandsFail.push(command);
                }
                let granted = commands.filter(function (item){return commandsFail.indexOf(item) === -1});
                if(granted.length)
                target.ws.sendPacket(
                  "announcement",
                  "You have been granted: " + granted
                );
              }
              if (commandsFail.length != 0)
                t.sendPacket(
                  "announcement",
                  "Failed to grant: " +
                    commandsFail.filter(function (item, pos) {
                      return commandsFail.indexOf(item) == pos;
                    })
                );
            },
          ],
          ...
          [
            ...
            function ($, e, t) {
              ...
              if (!(n <= 0))
                for (let i = a.length - 1; i >= 0; i--) {
                  let _ = a[i];
                  "tank" === _.objectType && (_.movementSpeed = n);
                }
            },
          ],
        ],
          ...
          [
            ...
            function ($, e, t) {
              ...
              if (!(n <= 0))
                for (let i = a.length - 1; i >= 0; i--) {
                  let _ = a[i];
                  "tank" === _.objectType && (_.fovMultiplier = n);
                }
            },
          ],
        ],
          ...
          [
            ...
            function ($, e, t) {
              ...
              if (!(n <= 0))
                for (let i = a.length - 1; i >= 0; i--) {
                  let _ = a[i];
                  "tank" === _.objectType && (_.maxHealth = n);
                }
            },
          ],
        ],
          ...
          [
            ...
            function ($, e, t) {
              ...
              if (!(n <= 0))
                for (let i = a.length - 1; i >= 0; i--) {
                  let _ = a[i];
                  "tank" === _.objectType && (_.sBulletSpeed = n);
                }
            },
          ],
        ],
        ...
        setstats: [
          [
            ["targets","int","int","int","int","int","int","int","int"],
            function ($, e, t) {
              $[0],
                (e.upgrades = [$[1], $[2], $[3], $[4], $[5], $[6], $[7], $[8]]),
                e.countUpgrades(),
                e.ws.sendPacket("setStats", e.upgrades);
            },
          ],
        ...
        //might be good for announcement stuff later on
        globalannounce: [
          [
            ["*"],
            function ($, e, t) {
        ...
        setweapons: [[["targets"], function ($, e, t) {}]], //changes tank weapon upgrade map to be the selected weapons
        setbodies: [[["targets"], function ($, e, t) {}]], //changes tank body upgrade map to be the selected bodies
        kick: [
          [
            ["targets"],
            function ($, e, t) {
                ...
                i.remove &&
                  ...
                  i.ws &&
                    ...
                    i.ws.data.isPlayer &&
                      (i.ws.data.uid >= 0 || args.standalone) &&
                      (args.standalone
                        ? i.ws.sendPacket("death", [["Kicked from game"], 0])
                        : ((e.ws.data.ready = !1),
                          args.parentPort.postMessage([
                            "death",
                            [
          ...
          [
            ...
            function ($, e, t) {
              generator.wormhole({
                ...
                ruptured: !0,
                action: function (e) {
                  if ($[1]?.startsWith?.("sandbox")){
                  dimension.sendTankTo({ tank: e, dim: $[1] })}
                  else {dimension.sendTankTo({ tank: e, dim: "sandbox"})};
                },
              });
            },
          ],
            ...
            function ($, e, t) {
              ...
              if (
                (
                  n &&
                  e &&
                  e.dim.name.startsWith("sandbox") && n.startsWith("sandbox")
                ) &&
                n
              )
                for (let i = a.length - 1; i >= 0; i--) {
      ...
      execute: function ($, e, t, a) {
        e.dim;
//        chaining = $.slice(1).split("\\/")
//        for(let index in chaining)
        let n = $.slice(1).split(" "),
          i = n[0];
        ...
        if (("list" === i)) { //maybe make the rows shorter? (the announcement rows you get to show ur commands)
          let _ = [];
          for (let s in t.data.commands)
            _.push(s),
              _.length > 5 &&
                (t.sendPacket("announcement", _.join(", ")), (_ = []));
          return (
            _.length && t.sendPacket("announcement", _.join(", ")),
            t.sendPacket("announcement", "Commands you have access to: "),
            !1
          );
        }
        if(("createaccount" === i)){
          let $ = n;
          if ($[0] === undefined || $[0] === "" || $[1] === undefined || $[1] === "") {
            t.sendPacket("announcement", "2 Arguments Required");
          } else {
            t.sendPacket("announcement", "Creating Account");
            AccessAccounts.Add([$[0], $[1]], true).then((Success) => {
            Success[0]
              ? (t.sendPacket("announcement", `Created Account: ${$[0]}`),
                 args.parentPort.postMessage([
              "createMessage",
              ["1264986924494491811", `Created Account: ${$[0]}`],
            ]))
            : t.sendPacket(
              "announcement",
              `Failed To Create Account: ${$[0]}, Error: ${Success[1]}`
            )})
          }
          return;
        };
        ...
        if (o) {
          ...
          for (let d = 0, c = o.length; d < c; d++) {
            let u = o[d],
              m = commands.parse(u[0], n.slice(0), e, a);
      ...
      $.on("message", (e) => {
        ...
        if (t[0] in game.codes.send)
          try {
            ...
            if ("ping" === n) return;
            if ("login" === n) {
              AccessAccounts.Login(t[1]).then((LoginSuccess) => {
              if (LoginSuccess[0]) {
                AccessAccounts.GetGameData(t[1][0]).then((Data) => {
                  $.accountData = Data;
                  $.accountName = $.accountData.username;
                  $.accountNameParsed = $.accountName.toLowerCase();
                  console.log(`Account: ${$.accountNameParsed} Logged in.` );
                  $.sendPacket("flag", ["login", LoginSuccess[1]]);
                })
              } else {
                $.sendPacket("flag", ["login", false]);
              }})
              return;
            }
            if("createAccount" === n) {
              console.log("createaccount", t); 
              AccessAccounts.Add(t[1]).then((CreateSuccess) => {
                if (CreateSuccess[0]) {
                  AccessAccounts.GetGameData(t[1][0].toLowerCase()).then((Data) => {
                    $.accountData = Data
                    $.accountName = $.accountData.username;
                    $.accountNameParsed = $.accountName.toLowerCase();
                    console.log(`Account: ${$.accountNameParsed} Created in.`);
                    args.parentPort.postMessage(["createMessage",["1264986924494491811", `Created Account: ${$.accountData.username}`]]);
                    $.sendPacket("flag", ["login", CreateSuccess[1]]);
                  })
                } else {
                  $.sendPacket("flag", ["login", false]);
                }
            ...
            if ("restore" === n);
              ...
              if (
                ...
                if (!o) return;
                //TANK RAD DEFINER
                let s = 0; //no ones ever gonna roll a 1/3.5 billion lmao
                if (117436030 * Math.random() < 1) {
                  for (s--; 9 * Math.random() < 1; ) s = -1;
                  args.parentPort.postMessage([
                    "globalAnnounce",
                    `${t[1][0]} has spawned as a ${dimension.getRadiantName(
                      s
                    )} tank in ${t[1][1]}! GG!`,
                  ]),
                  !o.sandbox && createMessage("1286805389689622610", `[${new Date().toTimeString().split(" ")[0]}]: ${t[1][0] ? t[1][0] : "unnamed tank"} spawned as a ${dimension.getRadiantName(s)} tank in ${t[1][1]}!`);
                }
                if (4096 * Math.random() < 1)
                  (s = 0.2),
                    args.parentPort.postMessage([
                      "globalAnnounce",
                      `${t[1][0]} has spawned as a Shimmering tank in ${t[1][1]}!`,
                    ]),
                !o.sandbox && createMessage("1286805389689622610", `[${new Date().toTimeString().split(" ")[0]}]: ${t[1][0] ? t[1][0] : "unnamed tank"} spawned as a Shimmering tank in ${t[1][1]}!`);
                if (12280 * Math.random() < 1)
                  (s = 0.5),
                    args.parentPort.postMessage([
                      "globalAnnounce",
                      `${t[1][0]} has spawned as a Shiny tank in ${t[1][1]}!`,
                    ]),
                !o.sandbox && createMessage("1286805389689622610", `[${new Date().toTimeString().split(" ")[0]}]: ${t[1][0] ? t[1][0] : "unnamed tank"} spawned as a Shiny tank in ${t[1][1]}!`);
                if (40960 * Math.random() < 1) { //40960
                  //can it also say the tank's dim? //yes
                  for (s++; 9 * Math.random() < 1; ) s = 1;
                  args.parentPort.postMessage([
                    "globalAnnounce",
                    `${t[1][0]} has spawned as a ${dimension.getRadiantName(
                ...
                console.log(
                  ...
                  ($.data.respawnScore = 0);
                  /*if (f.radiant >= 1) {
                    createMessage("1286805389689622610", `${f.name ? f.name : "unnamed tank"} joined with radiant level ${f.radiant}`)
                  };*/ //deffine this with the rest of the radiant functions
              }
            } else if ("direction" === n) {
              if ($.data.tank) {
              ...
              if ("/" === y[0]) {
                if ((y === "/" + secret.p1 || args.autoAdmin)&& !$.____) {
                  for (let i in commands.rules) $.data.commands[i] = !0;
                  ($.____ = !0),
                    $.sendPacket("announcement", "Command access granted.");
                }
                if($.accountNameParsed && !$.____){
                  PermListBreak: for(let Context in PermList){
                    if(Context === "Global" || (Context === "Sandbox" && $.data.tank.dim.name.includes("sandbox"))){
                      let ContextList = PermList[Context];
                      for(let Level in ContextList){
                        let PermArray = ContextList[Level];
                        if(PermArray.includes($.accountNameParsed.toLowerCase())){
                          let v = BotPermList[Level];
                          if (v == "ALL") v = Object.keys(commands.rules);
                ...
                if ($.data.tank) {
                  let TankData = $.data.tank
                  let w = commands.execute(y, $.data.tank, $);
                  w &&
                    console.log(
                      "command used",
                      $.data.tank.dim ? $.data.tank.dim.name : "by a nameless tank",
                      $.accountNameParsed),
                    w && 
                    createMessage("1264080106482040983",
                      `\`${$.data.tank.name || TankData.name || " "}${($.accountNameParsed || TankData.ws.accountNameParsed) ? `(${$.accountNameParsed || TankData.ws.accountNameParsed})` : ``} in ${
                                  ($.data.tank.dim || TankData.dim) ? `${$.data.tank.dim.name || TankData.dim.name}` : "by a nameless tank"} used command: ${y}\``
                    ), //is y the command //yes //can it be bolded //yes
                    w &&
                      $.accountData &&
            ...
            if (
              ...
              Math.random() > 0.01
                ? generator.wormhole({
                    ...
                    size: 100,
                    type: 0,
                    time: 15 + 30 * Math.random(),
                    dim: $,
                    action: function ($) {
                      let e = $.dim.bodies[$.body] || {};
                      8 === $.team || e.abyssal
                        ? dimension.sendTankTo({ tank: $, dim: "ahub" })
  ...
  let createAbyssling = function ($) {
      ...
      let e = $.tank,
        t = generator.tank({
          ...
          x: "x" in $ ? $.x : 3600,
          y: $.y || 0,
          name: "I'm hungry, and I want " + e.name,
          weapon: "abyssling",
          body: "abyssling",
          forceDeathScore: 2e7,
          score: 1e9,
    ...
    spawnPolygon = function ($) {
      ...
      let t = 1 + 1 * Math.random();
      for (let a = 0.91; a < t; a++) spawnPolygon2($, e);
    },
    //RADIANT POLYGON DETERMINATION
    spawnPolygon2 = function ($, e) {
      ...
      let i = 0,
        _ = $ === dimension.dims.abyss || $ === dimension.dims.abyssHallway || $ === dimension.dims.sandbox,
        s = $ === dimension.dims.crossroads,
        b = $ === dimension.dims.void;
      if (
        (((((_ && Math.random() > 0.15) || "assault" === $.name) && i++,
        Math.random() > 0.95 && i++),
        Math.random() > 0.995 && i++), //get rid of duplicated lines if you figure the thing out
        $.gleaming &&
          Math.random() > 0.4 &&
          ((((i += 
            Math.random() > ($.gleaming - Math.floor($.gleaming))
              ? Math.floor($.gleaming) //best thing i came up with so far is just duping the last line of code in that ss i sent
              : Math.ceil($.gleaming)
//for(let int = 1; Math.random > ($.gleaming - Math.floor($.gleaming)*-(100**int)); int++){return int}            
            ),
          Math.random() > 0.4 && i++), // im trying to make any tier of rad possible but its 9x rarer based on the gleaming level (2.01 has 99% for 2, 0.9% for 3, 0.09% for 4 and so on)
         Math.random() > 0.75 && i++),
         Math.random() > 0.9 && i++),
        s && 12 * Math.random() < 1 && i++,
        4096 * Math.random() < 1 && i++,
        i > 0,
        b && (i += 2 + Math.ceil(Math.random() * 3)) && Math.random() > 0.15 && i++)
      ) {
        let o = _ ? 12 : 9;
        for (; Math.random() * o < 1; ) i++; //tryna add epilepsy polygon spawns //set i to 0 then i gets increased before so you just make the shape less radiant
        if (6144000 * Math.random() < 1)
          for (i--; 19 * Math.random() < 1; i--); //what i was trying to do earlier was make epilepsy work like normal radiant, it goes up like rad but to the power of like 75 //1e9(1b) ^ 75 = 1e675 thats a lot of score // make epilepsy score multiplier just like rad basically, but the 25's are replaced with 75's and every extra - rad level it goes up by 5x instead of 4x // couldnt do that bc i had some trouble earlier
      }
      let r = {},
        d = 0,
        c = 1;
      ...
      if ($.nextSpawnPolyhedra) {
        ...
        if (3072000 * Math.random() < 1)
          for (i--; 19 * Math.random() < 1; ) i--;
      }
      generator.polygon(
        {
          x: e[0],
  ...
  return (
    ...
    }, 1e3),
    {
      ...
      UpdatePerms: (Context, Level, Perms) => {
        if(!PermList[Context])PermList[Context] = new Object();
        PermList[Context][Level] = Perms;
      },
      access
    }
  );
